...
function get()
{
	...
	if (empty($_GET)) {
		...
		$sql = 'SELECT * FROM pasta ORDER BY name';
		...
		if ($delete == 'true') { //delete == true required for delete
			...
			$sql = 'DELETE FROM pasta WHERE id = ? AND id IN (SELECT id FROM pasta)';
			$stmt = $pdo->prepare($sql);
			...
			if (isset($classification)) {
				...
				try {
					...
					$sql = 'SELECT * FROM pasta WHERE classification = ?';
					$stmt = $pdo->prepare($sql);
...
function post()
{
	...
	if (empty($_REQUEST['options'])) {
		...
		if (isset($_REQUEST['id'])) { //id required for edit
			...
			$id = $_REQUEST['id'];
			$sql = 'UPDATE pasta SET name = ?, description = ?, uses = ?, classification = ? WHERE id = ?';
			$stmt = $pdo->prepare($sql);
			...
			$sql = 'INSERT INTO pasta (name, description, uses, classification) VALUES (?, ?, ?, ?)';
			$stmt = $pdo->prepare($sql);
