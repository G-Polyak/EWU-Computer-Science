...
function get()
{
	...
	if (empty($_GET)) {
		...
		if($delete == 'true') { //delete == true required for delete
			...
			$sql = 'DELETE FROM pasta_noodles WHERE id = ? OR name = ? OR description = ? OR uses = ?';
			$stmt = $pdo->prepare($sql);
...
function post()
{
	...
	if (!empty($_REQUEST['id'])) { //id required for edit
		...
		$id = $_REQUEST['id'];
		$sql = 'UPDATE pasta_noodles SET name = ?, description = ?, uses = ? WHERE id = ?';
		$stmt = $pdo->prepare($sql);
		...
		$sql = 'INSERT INTO pasta_noodles (name, description, uses) VALUES (?, ?, ?)';
		$stmt = $pdo->prepare($sql);
