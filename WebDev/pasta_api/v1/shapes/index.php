...
function get()
{
	...
	if (empty($_GET)) {
		...
		$sql = 'SELECT * FROM pasta_shapes ORDER BY name';
		...
		if ($delete == 'true') { //delete == true required for delete
			...
			$sql = 'DELETE FROM pasta_shapes WHERE id = ? AND id IN (SELECT id FROM pasta_shapes)';
			$stmt = $pdo->prepare($sql);
			...
			if (isset($classification)) {
				...
				try {
					...
					$sql = 'SELECT * FROM pasta_shapes WHERE classification = ?';
					$stmt = $pdo->prepare($sql);
				...
				try {
					...
					$sql = 'SELECT * FROM pasta_shapes WHERE id = ? OR name = ? OR description = ? OR uses = ?';
					$stmt = $pdo->prepare($sql);
					$stmt->execute([$id, $name, $description, $uses]);
					$var = json_encode($stmt->fetchAll());
					if (isset($var)) {
...
function post()
{
	...
	if (empty($_REQUEST['options'])) {
		...
		if (isset($_REQUEST['id'])) { //id required for edit
			...
			$id = $_REQUEST['id'];
			$sql = 'UPDATE pasta_shapes SET name = ?, description = ?, uses = ?, classification = ? WHERE id = ?';
			$stmt = $pdo->prepare($sql);
			...
			$sql = 'INSERT INTO pasta_shapes (name, description, uses, classification) VALUES (?, ?, ?, ?)';
			$stmt = $pdo->prepare($sql);
